...
function logTest(component: string, action: string, status: 'PASS' | 'FAIL' | 'MANUAL', message: string, expectations: string[], actualBehavior?: string) {
  console.log(`\n${status === 'PASS' ? '✅' : status === 'FAIL' ? '❌' : '🔧'} ${component} - ${action}`);
  console.log(`   ${message}`);
...
markdown += '- ServiceTierSelector (recommendation calculator, tier selection)\n';
markdown += '- PMO Estimator Wizard (navigation, form submission, digital signature)\n';
