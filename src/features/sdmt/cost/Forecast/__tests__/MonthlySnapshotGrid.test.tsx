...
import type { SnapshotRow } from "../components/monthlySnapshotTypes";
import { filterSnapshotRows } from "../components/hooks/useMonthlySnapshotData";
...
describe("MonthlySnapshotGrid - Budget Allocation", () => {
  it("should allocate budget proportionally based on forecast when totalForecast > 0", () => {
    ...
    assert.ok(
      Math.abs(proj1.budget - 100) < 0.1,
      "Project 1 budget should be ~100"
    );
    assert.ok(
      Math.abs(proj2.budget - 200) < 0.1,
      "Project 2 budget should be ~200"
    );
    assert.ok(
      Math.abs(proj3.budget - 700) < 0.1,
      "Project 3 budget should be ~700"
    );

    // Verify total budget allocated equals monthBudget
    const totalAllocated = proj1.budget + proj2.budget + proj3.budget;
    assert.ok(
      Math.abs(totalAllocated - monthBudget) < 0.1,
      "Total allocated should equal monthBudget"
    );
  });
...
describe("MonthlySnapshotGrid - Grouping", () => {
  it("should group by project with rubros as children", () => {
    ...
    const projectMap = new Map<
      string,
      { forecast: number; actual: number; rubros: string[] }
    >();
    forecastData.forEach((cell) => {
      const projectId = cell.projectId || "unknown";
  ...
  it("should group by rubro with projects as children", () => {
    ...
    const rubroMap = new Map<
      string,
      { forecast: number; actual: number; projects: string[] }
    >();
    forecastData.forEach((cell) => {
...
describe("MonthlySnapshotGrid - Only Variance Filter", () => {
  it("should hide rows with zero variance when filter is enabled", () => {
    ...
    const filtered = rows.filter((row) => {
      const hasVariance =
        Math.abs(row.varianceBudget) > 0 || Math.abs(row.varianceForecast) > 0;
      return hasVariance;
    });
...
describe("MonthlySnapshotGrid - Cost Type Filter helper", () => {
  it("removes non-labor children when filter is set to labor", () => {
    const rows: SnapshotRow[] = [
      ...
      },
      {
        ...
        varianceBudgetPercent: 0,
        varianceForecast: -10,
        varianceForecastPercent: -11,
        status: "En Riesgo",
        costType: "non-labor",
      },
    ];

    const filtered = filterSnapshotRows({
      rows,
      costTypeFilter: "labor",
      searchQuery: "",
    ...
    assert.strictEqual(
      filtered.length,
      1,
      "Only group with labor costs should remain"
    );
    assert.strictEqual(
      filtered[0].children?.length,
      1,
      "Only labor child should remain"
    );
    assert.strictEqual(filtered[0].children?.[0].id, "child-labor");
  });
});
...
describe("MonthlySnapshotGrid - Denominator Zero Percent Rule", () => {
  it("should return null percent when budget (denominator) is 0", () => {
    ...
    assert.strictEqual(
      ...
      null,
      "Percent should be null when budget is 0"
    );
  });
  ...
  it("should return null percent when forecast (denominator) is 0", () => {
    ...
    assert.strictEqual(
      ...
      null,
      "Percent should be null when forecast is 0"
    );
  });
  ...
  it("should calculate percent correctly when denominator > 0", () => {
    ...
    assert.strictEqual(
      varianceBudgetPercent,
      20,
      "Percent should be 20 when variance is 200 and budget is 1000"
    );
  });
});
...
describe("MonthlySnapshotGrid - Performance", () => {
  it("should not trigger heavy recompute on unrelated prop change", () => {
    ...
    const forecastData: ForecastCell[] = Array.from(
      { length: 100 },
      (_, i) => ({
        line_item_id: `li${i}`,
        projectId: `proj${i % 10}`,
    ...
    assert.ok(
      computeTime < 100,
      `Computation time should be < 100ms, got ${computeTime}ms`
    );
...
describe("MonthlySnapshotGrid - Collapsed Mode", () => {
  ...
  it("should identify top 5 positive variances (overspend)", () => {
    ...
    assert.strictEqual(
      positive[0].varianceBudget,
      900,
      "Top variance should be 900"
    );
    assert.strictEqual(
      positive[4].varianceBudget,
      100,
      "Fifth variance should be 100"
    );
  });
  ...
  it("should identify top 5 negative variances (savings)", () => {
    ...
    assert.strictEqual(
      negative[0].varianceBudget,
      -1000,
      "Top savings should be -1000"
    );
    assert.strictEqual(
      negative[4].varianceBudget,
      -200,
      "Fifth savings should be -200"
    );
  });
  ...
  it("should handle sessionStorage fallback when user is unknown", () => {
    ...
    const storageKey = `monthlyGridCollapsed:${projectId}:${
      userEmail || "user"
    }`;
...
describe("MonthlySnapshotGrid - TDZ Regression Test", () => {
  it("should be importable without TDZ errors", async () => {
    ...
    try {
      ...
      assert.ok(
        module.MonthlySnapshotGrid,
        "MonthlySnapshotGrid should be exported"
      );
      assert.strictEqual(
