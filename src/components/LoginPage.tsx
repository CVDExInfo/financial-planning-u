...
function ResourceLink({
  ...
  return (
    <a
      ...
      className="flex items-center justify-between gap-4 rounded-lg border border-slate-200/60 bg-white/5 px-3 py-2 hover:bg-white/10 transition-colors dark:border-white/10 dark:bg-slate-800/40 dark:hover:bg-slate-800"
    >
...
export function LoginPage() {
  ...
  // Portal URLs baked by Vite at build time
          ...
          <Card className="relative overflow-hidden border border-slate-700/40 bg-slate-900/70 shadow-xl">
            <div className="relative grid gap-8 p-8 lg:grid-cols-4 lg:p-10">
              ...
              <div className="lg:col-span-3">
                ...
                <div className="mt-4">
                  <h2 className="text-3xl font-semibold">Ikusi · Central de Operaciones</h2>
                ...
                {/* three-tile balanced row (Recursos, Políticas, Soporte) */}
                <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {/* Left tile: Otros recursos (Accesos por rol + Sesión y seguridad) */}
                  ...
                  {/* Middle tile: Políticas y guías */}
                  <div className="rounded-xl border border-slate-700/40 p-4 bg-slate-900/60">
                    <p className="text-sm font-semibold text-emerald-200">Políticas y guías</p>
                    <p className="mt-3 text-sm text-slate-300">Accede a las guías operativas y procedimientos que rigen las actividades de operación.</p>
                  </div>

                  {/* Right tile: Soporte y contacto */}
                  <div className="rounded-xl border border-slate-700/40 p-4 bg-slate-900/60">
                    <p className="text-sm font-semibold text-emerald-200">Soporte y contacto</p>
                    <p className="mt-3 text-sm text-slate-300">¿Necesitas ayuda o acceso? Contacta a tu administrador o al equipo de soporte local.</p>
                  </div>
                </div>
              </div>

              {/* Right column: Accesos rápidos (buttons + resources below) */}
              <aside className="lg:col-span-1 flex flex-col gap-4">
                <div className="rounded-xl border border-slate-700/40 p-5 bg-slate-800/60">
                  ...
                  <p className="mt-1 text-xs text-slate-300">Accede con tu cuenta corporativa a las herramientas habilitadas según tu rol. Gestor de Actas y Prefacturas están disponibles como accesos directos.</p>
                ...
                <div className="flex flex-col gap-3">
                  <Button
                    type="button"
                    size="lg"
                    aria-label="Acceso a Finanzas SMO"
                  ...
                  <Button
                    ...
                    variant="ghost"
                    size="lg"
                    aria-label="Centrix - acceso directo"
                    className="h-12 w-full justify-between border border-slate-700/30 text-slate-100"
                ...
                {/* OTRAS PLATAFORMAS (middle column in your sketch) */}
                <div className="rounded-xl border border-slate-700/40 p-4 bg-slate-800/60">
                  ...
                  <div className="grid gap-2">
                    <ResourceLink href="https://ikusi.my.salesforce.com/" title="Login | Salesforce" subtitle="Acceso al CRM corporativo" ariaLabel="Login Salesforce - Acceso al CRM" />
                    <ResourceLink href="https://ikusi.service-now.com/colombia" title="SERVICENOW" subtitle="Gestión de incidencias y solicitudes (Colombia)" ariaLabel="ServiceNow - Gestión de incidencias Colombia" />
                ...
                {/* Recursos tile (left column content grouped earlier) */}
                <div className="rounded-xl border border-slate-700/40 p-4 bg-slate-800/60">
                ...
                {accessMessage ? <div className="rounded-lg border border-amber-600 bg-amber-900/10 px-4 py-3 text-sm text-amber-200 mt-2">{accessMessage}</div> : null}
              </aside>
            </div>
          </Card>
        </div>
