...
function ResourceLink({
  ...
  return (
    <a
      ...
      className="flex items-center justify-between gap-4 rounded-lg border border-slate-700/30 bg-slate-800/50 px-3 py-2 hover:bg-slate-800/60 transition-colors"
    >
      ...
      <ExternalLink className="h-4 w-4 text-slate-300 flex-shrink-0" aria-hidden="true" />
    </a>
  );
}
...
export function LoginPage() {
  ...
  const handlePortalAccess = (url: string, { requiresRoleCheck = false }: { requiresRoleCheck?: boolean } = {}) => {
    ...
    try {
      const newWin = window.open(url, "_blank") as Window | null;
      if (newWin) {
        try {
          (newWin as Window & { opener: Window | null }).opener = null;
        } catch {
          /* ignore cross-origin */
        }
      } else {
  ...
  useEffect(() => {
    ...
    const root = document.documentElement;
    if (previousAppearance.current === undefined) previousAppearance.current = root.dataset.appearance ?? (root.classList.contains("dark") ? "dark" : "light");
    ...
    <main className="relative min-h-screen overflow-hidden bg-gradient-to-b from-slate-950/40 via-slate-950 to-slate-900 text-slate-50">
      ...
      <div className="relative w-full px-4 py-12">
        <div className="mx-auto max-w-6xl">
          {/* HEADER — grid so logo left, centered H1, toggle right */}
          <div className="grid grid-cols-3 items-center gap-4 mb-6">
            ...
            <div className="text-center">
              ...
              <div className="mt-1 text-sm text-slate-300">
                ...
                <p className="mt-1 max-w-2xl mx-auto">Existimos para entregar excelencia con empatía y actitud que inspiran confianza</p>
              </div>
            </div>
          ...
          <Card className="relative overflow-hidden border border-slate-700/40 bg-slate-900/70 shadow-xl">
            <div className="relative grid gap-8 p-8 lg:grid-cols-4 lg:p-10">
              {/* Top-left large portal tile (spans cols 1-3) */}
              <div className="lg:col-span-3">
                ...
                <div className="mt-4">
                  <p className="text-sm text-emerald-200">Ikusi · Central de Operaciones</p>
                  <h2 className="mt-2 text-3xl font-semibold">Plataforma operativa para SDM, PMO, ingenieros y proveedores</h2>
              ...
              {/* SECOND ROW: Balanced 3-column grid - Recursos | Otras Plataformas | Accesos rápidos */}
              <div className="lg:col-span-3">
                <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  ...
                  <div className="rounded-xl border border-slate-700/40 p-4 bg-slate-900/60">
                    ...
                    <div className="text-sm text-slate-300 space-y-4">
                      <div>
                        <p className="font-medium">Sesión y seguridad</p>
                      ...
                      <div>
                        <p className="font-medium">Políticas y guías</p>
                        <p>Accede a las guías operativas y procedimientos que rigen las actividades de operación.</p>
                      </div>
                      <div>
                        <p className="font-medium">Soporte y contacto</p>
                        <p>¿Necesitas ayuda o acceso? Contacta a tu administrador o al equipo de soporte local.</p>
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Otras Plataformas */}
                  <div className="rounded-xl border border-slate-700/40 p-4 bg-slate-900/60">
                    <p className="text-sm font-semibold text-emerald-200 mb-3">Otras Plataformas</p>
                    <div className="grid gap-2">
                      <ResourceLink href="https://ikusi.my.salesforce.com/" title="Login | Salesforce" subtitle="Acceso al CRM corporativo" ariaLabel="Login Salesforce - Acceso al CRM corporativo" />
                      <ResourceLink href="https://ikusi.service-now.com/colombia" title="SERVICENOW" subtitle="Gestión de incidencias y solicitudes (Colombia)" ariaLabel="ServiceNow - Gestión de incidencias y solicitudes Colombia" />
                      <ResourceLink href="https://id.cisco.com/oauth2/default/v1/authorize?response_type=code&scope=openid%20profile%20address%20offline_access%20cci_coimemberOf%20email&client_id=cae-okta-web-gslb-01&state=e73wpl5CQD4G50dLMpSuqGjcpLc&redirect_uri=https%3A%2F%2Fccrc.cisco.com%2Fcb%2Fsso&nonce=pfDuXeO_o1BnKoOUdbwlNkx94k0P2BHYr5_zvC75EXw" title="CISCO CCW" subtitle="Portal Cisco para pedidos y licencias" ariaLabel="CISCO CCW - Portal Cisco para pedidos y licencias" />
                    </div>
                  </div>

                  {/* Accesos rápidos (buttons) */}
                  <div className="rounded-xl border border-slate-700/40 p-4 bg-slate-900/60">
                    <p className="text-sm font-semibold text-emerald-200 mb-3">Accesos rápidos</p>
                    <div className="flex flex-col gap-3">
                      <Button type="button" size="md" aria-label="Acceso a Finanzas SMO" className="h-12 w-full justify-between bg-emerald-600 text-slate-900 hover:bg-emerald-500" onClick={() => handleAccess("/", { requiresRoleCheck: false })} disabled={isLoading}>
                        <span className="flex items-center gap-2 font-semibold"><LogIn className="h-5 w-5" /> Acceso a Finanzas SMO</span>
                        <ArrowRight className="h-5 w-5" />
                      </Button>

                      <Button type="button" variant="secondary" size="md" aria-label="Gestor de Actas - acceso directo" className="h-12 w-full justify-between bg-slate-800 text-slate-100" onClick={() => handlePortalAccess(PMO_PORTAL_LOGIN)} disabled={isLoading}>
                        <span className="flex items-center gap-2 font-semibold"><Building2 className="h-5 w-5" /> Gestor de Actas</span>
                        <ArrowRight className="h-5 w-5" />
                      </Button>

                      <Button type="button" variant="outline" size="md" aria-label="Prefacturas Proveedores - acceso directo" className="h-12 w-full justify-between border-slate-700 bg-transparent text-slate-100" onClick={() => handlePortalAccess(PREFACTURAS_PORTAL_LOGIN)} disabled={isLoading}>
                        <span className="flex items-center gap-2 font-semibold"><FileSpreadsheet className="h-5 w-5" /> Prefacturas Proveedores</span>
                        <ArrowRight className="h-5 w-5" />
                      </Button>

                      <Button type="button" variant="ghost" size="md" aria-label="Horas Extras - módulo interno Ikusi" className="h-12 w-full justify-between border border-slate-700/30 text-slate-100" onClick={() => handleAccess("/horas-extras")} disabled={isLoading}>
                        <span className="flex items-center gap-2 font-semibold"><ExternalLink className="h-5 w-5" /> Horas Extras</span>
                        <ArrowRight className="h-5 w-5" />
                      </Button>

                      <Button type="button" variant="ghost" size="md" aria-label="Centrix - acceso directo" className="h-12 w-full justify-between border border-slate-700/30 text-slate-100" onClick={() => handlePortalAccess("https://newcentrix.labikusi.com/")} disabled={isLoading}>
                        <span className="flex items-center gap-2 font-semibold"><ExternalLink className="h-5 w-5" /> Centrix</span>
                        <ArrowRight className="h-5 w-5" />
                      </Button>
                    </div>
                  </div>
