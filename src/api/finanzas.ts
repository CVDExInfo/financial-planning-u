...
/**
 * Get rubros summary with aggregation from allocations/prefacturas
 * This is the server-side aggregation endpoint that returns rubros-like structure
 * even when the rubros table is empty, by aggregating allocations and prefacturas.
 */
export async function getRubrosSummary(
  projectId: string,
  baselineId?: string
): Promise<{
  rubro_summary: Array<{
  ...
  try {
    ...
    }>(path, { headers: buildAuthHeader() });

    return res.data;
  } catch (err) {
    if (err instanceof HttpError && (err.status === 404 || err.status === 405 || err.status === 501)) {
      // Endpoint not implemented yet
      return null;
    }
    throw toFinanzasError(err, "Unable to load rubros summary");
  }
}

/**
 * Get rubros with fallback to allocations/prefacturas aggregation
 * This is the client-side fallback that tries multiple sources:
 * 1. Rubros table (manual rubros)
 * 2. Server-side rubros-summary endpoint (aggregated allocations/prefacturas)
 * 3. Client-side aggregation of allocations/prefacturas (last resort)
 */
export async function getRubrosWithFallback(
  projectId: string,
  baselineId?: string
): Promise<LineItem[]> {
  if (!projectId) throw new FinanzasApiError("projectId is required");
  ...
  const rubros = await getProjectRubros(projectId);
  ...
  try {
    const summary = await getRubrosSummary(projectId, baselineId);
    if (summary && summary.rubro_summary && summary.rubro_summary.length > 0) {
      ...
      return summary.rubro_summary.map((r, idx) => ({
        ...
        created_by: "system",
        // Store monthly breakdown in metadata for forecast use
        metadata: {
          monthly: r.monthly,
          source: "allocations_aggregation",
        },
      }));
    }
  } catch (err) {
    logApiDebug("getRubrosSummary failed, falling back to client aggregation", err);
  }

  // 3) Client-side aggregation of allocations and prefacturas
  try {
    const [allocations] = await Promise.all([
      getAllocations(projectId),
    ]);
    ...
    const pushAllocation = (alloc: any) => {
    ...
    allocations.forEach(pushAllocation);
    ...
    logApiDebug("Client-side aggregation failed", err);
    return [];
  }
}
