...
- API base URL: FINZ_API_BASE, DEV_API_URL, VITE_API_BASE_URL, API_BASE_URL
- Bearer token: ID_TOKEN, FINZ_ID_TOKEN, FINZ_JWT, AUTH_TOKEN, COGNITO_ID_TOKEN,
  COGNITO_ACCESS_TOKEN, ACCESS_TOKEN, JWT_ID_TOKEN
- If no bearer token is provided, the script will authenticate against Cognito
  using: AWS_REGION, COGNITO_WEB_CLIENT, COGNITO_TESTER_USERNAME, COGNITO_TESTER_PASSWORD
...
def _resolve_bearer_token() -> Tuple[str, str]:
    for key in TOKEN_ENV_KEYS:
        ...
        if value:
            return value, key
    token = _authenticate_via_cognito()
    if token:
        return token, "cognito-auth"
    raise ValidationError(
        "Bearer token not found in environment. Provide Cognito tokens via ID_TOKEN/FINZ_ID_TOKEN or set COGNITO_TESTER_USERNAME/COGNITO_TESTER_PASSWORD."
    )
...
def _authenticate_via_cognito() -> str:
    username = os.getenv("COGNITO_TESTER_USERNAME", "").strip()
    password = os.getenv("COGNITO_TESTER_PASSWORD", "").strip()
    client_id = os.getenv("COGNITO_WEB_CLIENT", "").strip()
    region = os.getenv("AWS_REGION", "us-east-2").strip()

    if not (username and password and client_id):
        return ""

    client = boto3.client("cognito-idp", region_name=region, config=Config(retries={"max_attempts": 5, "mode": "standard"}))
    try:
    ...
    except Exception as exc:  # pragma: no cover - defensive logging for CI usage
        raise ValidationError(f"Cognito authentication failed: {exc}") from exc

    auth_result = resp.get("AuthenticationResult") or {}
    token = auth_result.get("IdToken") or auth_result.get("AccessToken")
    if not token:
        raise ValidationError("Cognito authentication succeeded but no token was returned")
    return token
