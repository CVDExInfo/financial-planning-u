...
test.describe("Finanzas projects flow", () => {
  ...
  test("navigates to projects manager and verifies API host", async ({
    page,
  }) => {
    ...
    const projectsResponse = await page
      .waitForResponse(
        (res) =>
          res.url().includes("/projects") && res.request().method() === "GET",
        { timeout: 20_000 }
      )
      .catch(() => null);

    if (projectsResponse) {
      expect(projectsResponse.url()).toContain(
        "https://pyorjw6lbe.execute-api.us-east-2.amazonaws.com"
      );
      expect([200, 401, 403, 501]).toContain(projectsResponse.status());
    }

    const projectTable = page
      .getByRole("row", { name: /Proyecto|Project/i })
      .first();
