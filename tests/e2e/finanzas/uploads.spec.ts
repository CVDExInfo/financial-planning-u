...
test.describe("Finanzas uploads", () => {
  ...
  test("attempts document upload and validates presigned PUT", async ({
    page,
  }) => {
    ...
    const uploadApiCall = await page.waitForResponse(
      (res) => res.url().includes("/uploads"),
      { timeout: 20_000 }
    );
    expect(uploadApiCall.url()).toContain(
      "https://pyorjw6lbe.execute-api.us-east-2.amazonaws.com/dev"
    );
    expect([200, 201]).toContain(uploadApiCall.status());

    const s3Put = await page.waitForResponse(
      (res) =>
        res.url().includes("amazonaws.com") && res.request().method() === "PUT",
      {
        timeout: 20_000,
      }
    );
