...
async function scanTableWithAudit(tableName: string): Promise<AuditTrailReport> {
...
async function checkAuditLog() {
  ...
  items.slice(0, 5).forEach((entry, idx) => {
    ...
    console.log(`   Source: ${entry.source || 'N/A'}, IP: ${entry.ip_address || 'N/A'}`);
    console.log();
  });
}
...
async function main() {
  ...
  for (const table of tables) {
    try {
      ...
      if (report.totalItems > 0) {
        const auditCoverage = Math.round((report.withBoth / report.totalItems) * 100);
        console.log(`  Audit Trail Coverage: ${auditCoverage}% (${report.withBoth}/${report.totalItems} items)`);
        
        if (auditCoverage < 100) {
          console.log(`  ⚠️  Missing audit fields:`);
          console.log(`     - created_by: ${report.totalItems - report.withCreatedBy} items`);
          console.log(`     - created_at: ${report.totalItems - report.withCreatedAt} items`);
        }
        
        // Show sample with audit fields
        const sampleWithAudit = report.samples.find(s => s.created_by && s.created_at);
        if (sampleWithAudit) {
          console.log(`  📝 Sample:`);
          console.log(`     ID: ${sampleWithAudit.id || sampleWithAudit.codigo || 'N/A'}`);
  ...
  console.log("  1. All new data entries should include created_by and created_at fields");
  console.log("  2. All write operations should log to finz_audit_log");
  console.log("  3. Audit log should include: action, user, timestamp, resource_type, resource_id");
  console.log("  4. Consider adding IP address and user agent for security tracking");
  console.log("  5. Implement audit log retention policy (e.g., TTL after 1 year)");
}
